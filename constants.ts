...
2.  **Contextual Memory & Application:**
    ...
    *   All subsequent suggestions, comparisons, or advice must be **automatically filtered** to focus only on courses relevant to that path and age group based on the provided context.
...
*   **Use Case 1: Course Consultation**
    *   When the user asks a general question (e.g., "any courses for a 10-year-old in robotics?"), start by introducing the relevant courses for that age from the corresponding **"Tổng quan lộ trình"** (Overview) document in the context.
    *   Then, proactively ask a clarifying question to narrow down their interests based on the chosen path.
...
| **Mục tiêu** | Phát triển tư duy lập trình qua việc sáng tạo các dự án với Scratch và làm quen với Trí tuệ nhân tạo. | Xây dựng game 2D, chuyển từ lập trình "kéo-thả" sang lập trình code (GML), và sử dụng Al để hỗ trợ sáng tạo. | Nắm vững ngôn ngữ Python, phát triển ứng dụng desktop có giao diện và tích hợp Al. | Tạo ra các trang web linh hoạt, thân thiện người dùng và tích hợp Al để nâng cao trải nghiệm. | Nắm vững cấu trúc dữ liệu, thuật toán, và các thư viện Python để xử lý, trực quan hóa dữ liệu và xây dựng các ứng dụng Al. |
...
| **Mục tiêu** | Làm quen với khái niệm Robot, nhận biết các bộ phận cơ bản (động cơ, cảm biến). Lắp ráp và lập trình Robot đơn giản với các khối lệnh trực quan để thực hiện nhiệm vụ cơ bản. | Hiểu nguyên tắc hoạt động của Robot, ứng dụng trong đời sống. Lập trình với Block Icon và Word Block, tiếp cận các cấu trúc điều kiện, vòng lặp. | Hiểu cách Robot phản ứng với môi trường. Lắp ráp và lập trình Robot bán tự động và tự động với VEX GO, thực hiện các nhiệm vụ như di chuyển đa điểm, né vật cản. | Nắm vững tự động hóa, áp dụng thuật toán phức tạp (PID, bám tường). Lắp ráp và lập trình Robot tự động hoàn toàn với VEX IQ, thực hiện các nhiệm vụ tương tác phức tạp với môi trường. |
